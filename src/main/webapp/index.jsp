<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8" errorPage="/WEB-INF/jsp/errorPage.jsp"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@ page import="by.epam.jwd.controller.command.PagePath" %>
<%@ page import="by.epam.jwd.controller.command.Command" %>

<!DOCTYPE html>
<html>
<body>
	<p> <a href="Controller?command=GoToSignIn"> Sign In </a> </p>
	<h2>Minsk Motor depot</h2>
	<p> <a href="Controller?command=GoToPresentationPage"> Our cars </a> </p>

	<p>Our contacts +375291111111</p>


<p></p>
</body>
</html>
