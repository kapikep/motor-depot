...
<c:if test="${page != 1}">
    <a href="customer?command=${param.command}&page=${page - 1}"><fmt:message bundle="${loc}" key="pagination.prev"/></a>
</c:if>
<c:forEach var="i" items="${numPages}">
    <c:choose>
        <c:when test="${i==page}">
            <b><a href="customer?command=${param.command}&page=${i}"><c:out value="${i}"/></a></b>
        </c:when>
        <c:otherwise>
            <a href="customer?command=${param.command}&page=${i}"><c:out value="${i}"/></a>
        </c:otherwise>
    </c:choose>
</c:forEach>
